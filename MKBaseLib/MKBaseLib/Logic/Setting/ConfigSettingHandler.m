 ...
 *  @param on YES开，否则关
 */
+ (void)setAutoAnswerCallOn:(BOOL)on{
    [[NSUserDefaults standardUserDefaults] setBool:on forKey:kCZAutoAnswerCallOnOff];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
 ...
 *  @return YES是，否则不回拨
 */
+ (BOOL)isAutoAnswerCallOn{
    NSUserDefaults *userDefault=[NSUserDefaults standardUserDefaults];
    if ([userDefault objectForKey:kCZAutoAnswerCallOnOff]) {
        return [userDefault boolForKey:kCZAutoAnswerCallOnOff];
    }
    return NO;
}
