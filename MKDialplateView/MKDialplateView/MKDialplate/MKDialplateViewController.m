...
- (void)initUI
{
    ...
    self.showDialNumberView.backgroundColor = [UIColor clearColor];
    ...
    //创建内容区域，承载通话记录和搜索结果
    //_centerContentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 113)];
    _centerContentView = [[UIView alloc] initWithFrame:self.view.bounds];
    _centerContentView.backgroundColor = [UIColor clearColor];
    [self.view addSubview:_centerContentView];
    
    //搜索联系人列表
    self.searchContactTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, _centerContentView.frame.size.height) style:UITableViewStylePlain];
    ...
    [_centerContentView addSubview:self.searchContactTable];
    
    //通话记录列表
    self.callRecordsListVc = [[MKCallRecordViewController alloc] init];
    self.callRecordsListVc.view.frame =CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, _centerContentView.frame.size.height);
    [self addChildViewController:self.callRecordsListVc];
    [_centerContentView addSubview:self.callRecordsListVc.view];
    
    //拨号盘
    //self.dialplateView = [[MKDialplateView alloc] initWithPosition:CGPointMake(0, self.view.bounds.size.height-285)];
    self.dialplateView = [[MKDialplateView alloc] initWithPosition:CGPointMake(0, self.view.bounds.size.height-236)];
    //self.dialplateView = [[MKDialplateView alloc] initWithPosition:CGPointMake(0, [UIScreen mainScreen].bounds.size.height-236-113)];
    ...
    //隐藏键盘通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveHideDialplate) name:kHideDialPlateNotificationChanged object:nil];
    //显示键盘通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveShowDialplate) name:kShowDialPlateNotificationChanged object:nil];
    
    //值监听
    [self.phoneNumberLabel addObserver:self forKeyPath:@"text" options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:nil];
    isShowBottomView=NO;
}

//隐藏键盘通知
- (void)receiveHideDialplate{
...
//显示键盘通知
- (void)receiveShowDialplate{
    if ([self.phoneNumberLabel.text length]>0){
        [self changeViewModelToDialInput];
        //显示拨打
        if(!isShowBottomView){
            isShowBottomView=YES;
            [self showDialplateViewStatus:YES];
        }
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if (object==self.phoneNumberLabel) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:KDialNumberViewValueChanged object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self.phoneNumberLabel.text,@"text", nil]];
...
- (void)dealloc
{
    [self.phoneNumberLabel removeObserver:self forKeyPath:@"text"];
...
- (void)UpdateUI
{
    ...
    rect.origin.y = self.view.frame.size.height - 236;
...
- (void) DialplateViewNumberInput:(NSInteger)number
{
    ...
    if (number==10) {
       numStr=[NSString stringWithFormat:@"%@*",self.tempnumdata];
    }
    if (number==12) {
       numStr=[NSString stringWithFormat:@"%@#",self.tempnumdata];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_isGoingtoNewContact)
    {
        ...
        if (cell == nil)
        {
            ...
            cell.backgroundColor = [UIColor colorWithRed:247/255.0 green:247/255.0 blue:241/255.0 alpha:1.0];
            cell.selectedBackgroundView.backgroundColor = [UIColor blackColor];
            cell.selectedBackgroundView.alpha = 0.05;
        }
        if (indexPath.row == 0)
        {
        ...
        lineView.backgroundColor =[UIColor colorWithRed:226/255.0 green:226/255.0 blue:220/255.0 alpha:1.0];
    ...
    else
    {
        ...
        operationBtn.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-42, (57-22)/2, 22, 22);
        operationBtn.backgroundColor = [UIColor clearColor];
        
        [operationBtn setImage:[MKDialplateUtils getImageFromResourceBundleWithName:@"searchcontact_info_normal@2x" type:@"png"] forState:UIControlStateNormal];
        ...
        if (indexPath.row == 0)
        {
            ...
            lineView1.backgroundColor = [UIColor colorWithRed:226/255.0 green:226/255.0 blue:220/255.0 alpha:1.0];
...
- (void)peoplePickerNavigationController:(ABPeoplePickerNavigationController*)peoplePicker didSelectPerson:(ABRecordRef)person
{
    ...
    ABMultiValueAddValueAndLabel(multiPhone, (__bridge CFTypeRef)phoneNumer, kABPersonPhoneMobileLabel, NULL);
